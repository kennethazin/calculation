  ...
  try {
    const dataSource = await viewer.dataSources.add(
      Cesium.CzmlDataSource.load(
        "./orbital_calculations/saturn_v_trajectory_orientation.czml"
      )
    );
    ...
    if (satellite) {
      ...
      ); // Adjusted to rotate around the Y-axis
      const rotationQuaternion =
        Cesium.Quaternion.fromRotationMatrix(rotationMatrix);

      satellite.orientation = new Cesium.CallbackProperty((time, result) => {
      ...
      console.error(
        "Satellite entity with ID 'SaturnV' not found in the CZML file."
      );
    }
    ...
    console.error(
      "Failed to load the CZML file. Please check the file path and ensure the file is accessible.",
      error
    );
  }
}
